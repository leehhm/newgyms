...
<body>
	<form action="${contextPath}/owner/review/orderReviewList.do" method="post"
			...
			<div class="con">
				<div id="contain">
					...
					<div id="contain_right">
						...
						<c:choose>
							<c:when test="${empty ownerReviewList}">
								<table id="order_detail2" align=center>
									...
									<tr>
										<td colspan="7" style="color: blue;">작성한 이용후기가 없습니다. 😂</td>
									</tr>
								</table>
							</c:when>
							<c:otherwise>
								<table id="order_detail2" align=center>
									...
									<c:forEach var="item" items="${ownerReviewList}"
										varStatus="status">
										...
										<tr class="owner_review_title" style="cursor: pointer;">
											...
											<!-- 상품명 -->
											<td width="25%" style="font-size: 18px; text-align: center;">${item.product_name }<br>
											</td>
											<!-- 제목 -->
										...
										<tr class="owner_review_hidden">
											...
											<td id="gray_color"
												...
												<br>
												<!-- 이미지 --> 
												<c:forEach var="ril" items="${reviewImageList}" varStatus="status">
													<c:if test="${ril.review_no == item.review_no}">
														<img alt="이용후기 이미지" style="width:250px; display: inline; margin: 0px auto;"
				...
				<div style="text-align: center;">
					...
					<c:forEach var="page" begin="1" end="${Math.ceil(maxnum/5)}"
						...
						<a
							href="${contextPath}/owner/review/orderReviewList.do?chapter=${section_num}&center_name=${center_name}&firstDate=${firstDate}&secondDate=${secondDate}&text_box=${text_box}">${section_num }</a>
					</c:forEach>
